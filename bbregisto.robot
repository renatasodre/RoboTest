...
Clicar no Botão Registrar    
    ...
    Run Keyword And Ignore Error    Execute JavaScript    document.querySelector('button[type="button"]:contains("Registrar")').click()
...
Clicar no Botão de Saldo
    ...
    Run Keyword If    ${criar_com_saldo} and 'ativo' not in '${estado_atual}'    Click Element    ${label_saldo}
    Run Keyword If    not ${criar_com_saldo} and 'ativo' in '${estado_atual}'    Click Element    ${label_saldo}
